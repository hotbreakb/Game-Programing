...
HRESULT DemoApp::CreateDeviceResources()
{
	...
	if (!m_pRT)
	{
		...
		/*if (SUCCEEDED(hr))
		{
			hr = LoadBitmapFromResource(m_pRT, m_pWICFactory, L"IDB_AIR", L"PNG", &m_pFernBitmap);
		}

		if (SUCCEEDED(hr))
		{
			hr = LoadBitmapFromResource(m_pRT, m_pWICFactory, L"IDB_AIRMASK", L"PNG", &m_pBitmapMask);
		}*/
...
HRESULT DemoApp::OnRender()
{
	...
	if (SUCCEEDED(hr))
	{
		...
		// WCHAR szText[100];
		/*swprintf_s(szText, L"컴퓨터 속도:%f\n플레이어 속도:%f\n", COM_VELOCITY, USER_VELOCITY);
		...
		//m_pRT->FillOpacityMask(m_pBitmapMask, m_pFernBitmapBrush, D2D1_OPACITY_MASK_CONTENT_GRAPHICS);
